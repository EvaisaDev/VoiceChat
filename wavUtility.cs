...
public class WavUtility
{
	...
	public static byte[] FromFloatArray(int samples, int channels, int frequency, float[] data)
	{
		...
		// total file size = 44 bytes for header format and audioClip.samples * factor due to float to Int16 / sbyte conversion
		int fileSize = samples * BlockSize_16Bit + headerSize; // BlockSize (bitDepth)

		// chunk descriptor (riff)
		WriteFileHeader(ref stream, fileSize);
		// file header (fmt)
		WriteFileFormat(ref stream, channels, frequency, bitDepth);
		// data chunks (data)
		WriteFileData2(ref stream, samples, data, bitDepth);
	...
	private static int WriteFileData2(ref MemoryStream stream, int samples, float[] data, UInt16 bitDepth)
	{
		int count = 0;
		int total = 8;
		...
		int subchunk2Size = Convert.ToInt32(samples * BlockSize_16Bit); // BlockSize (bitDepth)
